...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, softmax=False, ignore=None):
        ...
        target = self._one_hot_encoder(target) if self.n_classes != 1 else target
        if weight is None:
        ...
        if self.n_classes == 1:
            # print(
            #     inputs.shape, target.shape, ignore.shape if ignore is not None else ""
            # )
            dice = self._dice_loss(inputs, target, ignore)
            return 1 - dice.item()
...
def convert_googledrive_url(url):
    return url.replace(
...
def cacl_iou(y, y_pred):
    ...
    union = y.sum() + y_pred.sum() - intersection
